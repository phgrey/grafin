...
from graphin.adapters.cordis_ipc_bridge import CordisIPCBridge
...
def get_adapter_by_name(name: str) -> IFrameworkAdapter:
    ...
    elif name_lower in ("cordis", "cordis_adapter"):
        return CordisAdapter()
    else:
        raise ValueError(f"Unknown adapter name: '{name}'. Available: langgraph, crewai, semantic_kernel, graphin_yaml, cordis.")
