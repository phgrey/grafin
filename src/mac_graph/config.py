...
class AppConfig(BaseModel):
    ...
    source_dir: str = Field(default="data/source", description="Source directory for input MD files")
    output_dir: str = Field(default="data/results", description="Output directory for tagged MD files")
    confidence_threshold: float = Field(
        default=0.75, description="Confidence threshold below which HITL interrupt occurs (0.0 to 1.0)"
    )
...
def load_config(config_path: Optional[str] = None) -> AppConfig:
    ...
    if os.getenv("MAC_GRAPH_PROVIDER"):
        config_dict["provider"] = os.getenv("MAC_GRAPH_PROVIDER")
    if os.getenv("MAC_GRAPH_CONFIDENCE_THRESHOLD"):
        try:
            config_dict["confidence_threshold"] = float(os.getenv("MAC_GRAPH_CONFIDENCE_THRESHOLD"))
        except ValueError:
            pass
