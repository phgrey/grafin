...
def split_markdown_into_sections(filename: str, content: str) -> List[TextChunk]:
    ...
    if not matches:
        # If no headings found, split by double newlines into logical paragraph blocks
    ...
    for i, match in enumerate(matches):
        ...
        # Remove the heading line itself from the body content for clean analysis
        lines = section_text.splitlines()
        body_lines = lines[1:] if len(lines) > 1 else []
        body_content = "\n".join(body_lines).strip()

        # Skip document title header if it has no body content and sub-headings exist
